...
<script language="javascript" type="text/javascript">
    ...
    function tableOrdering( order, dir, task )
    {
        ...
        form.filter_shifts_order.value    = order;
        form.filter_shifts_order_Dir.value   = dir;
...
<form action="index.php" method="post" name="adminForm" id="adminForm" >
    <div id="editcell">
        <table class='table'>
            <thead>
                <tr>
                    <th width="5"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_SHIFTS_TMPL_DEFAULT_ID'); ?></th>
                    <th width="20"><input type="checkbox" name="checkall-toggle" value title="Check All" onclick="Joomla.checkAll(this)" /></th>
                    <th style="text-align: left;"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_SHIFTS_TMPL_DEFAULT_SHIFT'); ?></th>
                    <th style="text-align: left;"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_SHIFTS_TMPL_DEFAULT_COMPANY'); ?></th>
                </tr>
            </thead>
            <?php
            $k = 0;
            for ($i=0, $n=count( $this->items ); $i < $n; $i++)
            {
                ...
                $link = JRoute::_( 'index.php?option=com_jobgroklist&controller=shift&task=edit&cid[]='. $row->id );
                ?>
            <tr class="<?php echo "row$k"; ?>">
                ...
                <td>
                    <a href="<?php echo $link; ?>"><?php echo $row->shift; ?></a>
                </td>
                <td>
                        <?php
                        $db = JFactory::getDBO();
    ...
    <input type="hidden" name="controller" value="shift" />
