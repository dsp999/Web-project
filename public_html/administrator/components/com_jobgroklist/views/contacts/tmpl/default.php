 ...
 *
 * Subversion Details
 * $Date: 2013-11-19 22:40:55 -0600 (Tue, 19 Nov 2013) $
 * $Revision: 5614 $
 * $Author: jobgrok $
 *
...
<script language="javascript" type="text/javascript">
    ...
    function tableOrdering( order, dir, task )
    {
        ...
        form.filter_contacts_order.value    = order;
        form.filter_contacts_order_Dir.value   = dir;
...
<form action="index.php" method="post" name="adminForm" id="adminForm" >
    <div id="editcell">
        <table class='table'>
            <thead>
                <tr>
                    <th width="5"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_CONTACTS_TMPL_DEFAULT_ID'); ?></th>
                    <th width="20"><input type="checkbox" name="checkall-toggle" value title="Check All" onclick="Joomla.checkAll(this)" /></th>
                    <th style="text-align: left;"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_CONTACTS_TMPL_DEFAULT_CONTACT'); ?></th>
                    <th style="text-align: left;"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_CONTACTS_TMPL_DEFAULT_EMAIL_ADDRESS'); ?></th>
                    <th style="text-align: left;"><?php echo JTEXT::_('COM_JOBGROKLIST_VIEWS_CONTACTS_TMPL_DEFAULT_COMPANY'); ?></th>
                </tr>
            </thead>
            <?php
            $k = 0;
            for ($i=0, $n=count( $this->items ); $i < $n; $i++)
            {
                ...
                $link = JRoute::_( 'index.php?option=com_jobgroklist&controller=contact&task=edit&cid[]='. $row->id );
                ?>
            <tr class="<?php echo "row$k"; ?>">
                ...
                <td>
                    <a href="<?php echo $link; ?>"><?php echo $row->contact; ?></a>
                </td>
                <td>
                    <?php
                        $emails = explode(";",$row->contact_email);
                        foreach ($emails as $e) {
