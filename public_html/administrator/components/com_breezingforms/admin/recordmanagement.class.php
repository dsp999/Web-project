...
class bfRecordManagement{
    ...
    function getTableBar(){
        ...
        $out .= '<div id="bfAvailableFields">'.bf_alert('Column selection in full version, only', 'http://crosstec.de/en/extensions/joomla-forms-download.html').'</div>'."\n";
        $out .= '</div>';
        $out .= '</div>';
    ...
    function listRecords(){
        ...
        ?>
            <?php echo bf_alert('Get BreezingForms Full Version', 'http://crosstec.de/en/extensions/joomla-forms-download.html');?>
            <?php echo bf_alert('More features, no footers, no messages', 'http://crosstec.de/en/extensions/joomla-forms-download.html');?>
            <br />
    ...
    function getListRecords(){
        ...
        if($searchdatefrom == '' && $searchtimefrom != ''){
            $searchdatefrom = date('Y-m-d',strtotime($now_date)).' '.date('H:i:s',strtotime('1970-01-01 '.$searchtimefrom));
        } else if($searchdatefrom != '' && $searchtimefrom != ''){
            $searchdatefrom = date('Y-m-d',strtotime($searchdatefrom)).' '.date('H:i:s',strtotime('1970-01-01 '.$searchtimefrom));
        } else if($searchdatefrom != '' && $searchtimefrom == ''){
            $searchdatefrom = date('Y-m-d',strtotime($searchdatefrom)).' 00:00:00';
        }
        ...
        if($searchdateto == '' && $searchtimeto != ''){
            $searchdateto = date('Y-m-d',strtotime($now_date)).' '.date('H:i:s',strtotime('1970-01-01 '.$searchtimeto));
        } else if($searchdateto != '' && $searchtimeto != ''){
            $searchdateto = date('Y-m-d',strtotime($searchdateto)).' '.date('H:i:s',strtotime('1970-01-01 '.$searchtimeto));
        } else if($searchdateto != '' && $searchtimeto == ''){
            $searchdateto = date('Y-m-d',strtotime($searchdateto)).' 23:59:59';
        }
        ...
        if($searchdatefrom != '' && version_compare($this->version, '3.2', '>=')){
            ...
            $offset = $this->tz->getOffset($date_);
            // revert back to UTC by adding the offset and add another offset to make the search possible
            $searchdatefrom = date('Y-m-d H:i:s', $date_->format('U') + $offset + $offset);
        }
        
        if($searchdateto != '' && version_compare($this->version, '3.2', '>=')){
            $date_ = JFactory::getDate($searchdateto, $this->tz);
            $offset = $this->tz->getOffset($date_);
            $searchdateto = date('Y-m-d H:i:s', $date_->format('U') + $offset + $offset);
        }
        ...
        try{
            ...
            foreach($result['Records'] As $record){
                ...
                if(version_compare($this->version, '3.2', '>=')){
                    ...
                    $result['Records'][$i]['bfrecord_submitted'] = $date_->format('Y-m-d H:i:s');
                }
                $i++;
            }
        }catch(Exception $e){
		...
		$recs = $db->loadObjectList();
                ...
                foreach($recs As $rec){
                    ...
                    if(version_compare($this->version, '3.2', '>=')){
                        ...
                        $recs[$i]->submitted = $date_->format('Y-m-d H:i:s');
                    }
                    $i++;
                }
        ...
        function exportCsv()
	{
		...
		for($r = 0; $r < $recsSize; $r++) {
                        ...
                        if(version_compare($this->version, '3.2', '>=')){
                            $date_ = JFactory::getDate($rec->submitted, $this->tz);
                            $rec->submitted = $date_->format('Y-m-d H:i:s');
                        }
