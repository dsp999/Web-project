...
* $Id: mod_cblogin.php 1901 2012-11-01 15:39:46Z beat $
*
* @version 1.2
...
if ( $_CB_framework->myId() ) {
	...
	global $cbSpecialReturnAfterLogout;
	if ( isset( $cbSpecialReturnAfterLogout ) ) {
		$logout		=	$cbSpecialReturnAfterLogin;
	}
...
*/
	...
	if ( ( ! $horizontal ) && ( $jVersion == 2 ) ) {
		echo '<div id="form-login-username" class="form-group"><div class="input-group">';
	}
	if ( ! $compact ) {
		//echo '<span id="mod_login_usernametext1'.$id_sfx.'">'.$txtusername.'</span>';
		echo '<span class="input-group-addon" id="mod_login_usernametext1'.$id_sfx.'"><i class="fa fa-user tip" title="Username"></i></span>';
	}
	if ($horizontal) {
		echo "&nbsp;\n";
	...
	echo '<span><input type="text" name="username" placeholder="Email Address" id="mod_login_username'.$id_sfx.'" class="inputbox'.$class_sfx.' input-medium" size="'.$name_lenght.'"' . $bgstyleUser . ' /></span>';
	if ($horizontal) {
		echo "&nbsp;\n";
	} elseif ( $jVersion == 2 ) {
		echo '</div></div>';
		echo '<div id="form-login-password" class="form-group"><div class="input-group">';
	} else {
		echo "<br />\n";
	}
	...
	if (!$compact) {
		...
		echo '<span class="input-group-addon" id="mod_login_passwordtext'.$id_sfx.'"><i class="fa fa-lock tip" title="Password"></i></span>';
	}
	if ($horizontal) {
		echo "&nbsp;";
	} elseif ( ( $jVersion != 2 ) && ( ! $compact ) && ( $show_username_pass_icons != 2 ) ) {
	...
	echo '<span><input type="password" name="passwd" placeholder="Password" id="mod_login_password'.$id_sfx.'" class="inputbox'.$class_sfx.' input-medium" size="'.$pass_lenght.'"' . $bgstylePass . ' /></span>';
	if ($horizontal) {
		echo "&nbsp;\n";
	} elseif ( $jVersion == 2 ) {
		echo '</div></div>';
	...
	switch ($remember_enabled) {
		...
		case 3:
			...
			if ( $jVersion == 2 ) {
				if ( ! $horizontal ) {
					echo '<div id="form-login-remember" class="form-group">';
				}
				echo $remInput;
				echo $remLabel;
				if ( ! $horizontal ) {
	...
	echo '<span class="cbLoginButtonSpan mn-modal-controls">';
	echo '<input type="submit" name="Submit" class="btn-primary btn button'.$class_sfx.'" value="' . $buttonValue . '"' . $buttonStyle . ' />';
	echo '</span>';
	...
	if ( $listFormatted ) {
		 echo '<ul class="cbLoginLinksList unstyled">';
	}
