...
* v2.5.19 Mar 24, 2012 added support for zoom level and multiple InfoWindows with KML files!
* v2.5.20 Apr 20, 2013 changed onload() to addEvent('load')
* v3.0.21 Oct 14, 2013 added css style img { max-width: none; }
*/
		...
		if (preg_match_all( $regex, $contents, $matches, PREG_SET_ORDER )) {
			...
			foreach($matches as $matches2) {
				...
				if(defined('_VALID_MOS')) {
					...
					JFactory::getDocument()->addStyleDeclaration($this->setup_css($map_id));
				}
			...
			global $joomla_ver;
			#print "bp301. joomla_ver = $joomla_ver<br>";

			if ($joomla_ver=='3.0') {
				$load_gmap_script = "jQuery(document).ready(function() {
    load_{$this->mod}();
});";
			} else {
				//onload_{$this->mod}(load_{$this->mod});
//window.onunload = GUnload;
//window.addEvent('load', load_{$this->mod});
				$load_gmap_script = "window.addEvent('load', load_{$this->mod});";
			}
...
";
	...
	function setup_css($map_id) {
		#print "bp201. mod($this->mod) map_id($map_id)<br>";
		#$output = '#gmap_gmap0 img { max-width: none; } ';
		$output = "#{$this->mod}_gmap{$map_id} img { max-width: none; } ";
		#$output = '';
		return $output;
	}
