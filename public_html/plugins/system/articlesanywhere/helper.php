...
class plgSystemArticlesAnywhereHelper
{
	...
	function processArticle($id, $art, $text = '', $type = 'article', $ignores = array(), $count = 0, $first = 0)
	{
		...
		if (preg_match_all($ifregex, $text, $ifs, PREG_SET_ORDER) > 0)
		{
			foreach ($ifs as $if)
			{
				$script = html_entity_decode(trim($if['1']));
				/*if (preg_match('#(^.*?[><=]\s*([\'"]))(.*)\2#si', $script, $match))
				{
					$script = str_replace($match['0'], $match['1'] . html_entity_decode($match['3']) . $match['2'], $script);
				}*/
				$script = str_replace('=', '==', $script);

				$script = '$article->' . preg_replace('#\s*(&&|&&|\|\|)\s*#', ' \1 $article->', $script);
				$script = str_replace('$article->!', '!$article->', $script);

				$script = 'return ( ' . $script . ' ) ? 1 : 0;';
				// trim the text that needs to be checked and replace weird spaces
				$script = preg_replace('#(\$article->[a-z0-9-_]*)#', 'trim(str_replace(chr(194) . chr(160), " ", \1))', $script);

				$temp_PHP_func = create_function('&$article', $script);
			...
			)
			{
				...
				if ($extra)
				{
					...
					if ($strip)
					{
						...
						$str = preg_replace('#(<' . 'style[^a-z0-9].*?</style>)#si', ' ', $str);
						// remove other tags
